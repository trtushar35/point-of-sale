...
const applyFilter = () => {
    router.get(route('backend.invoice.index'), filters.value, { preserveState: true });
};
        ...
        <div
            ...
            <div
                ...
                <div class="grid w-full grid-cols-1 gap-2 md:grid-cols-5">
                    ...
                    <!-- Color Filter -->
                    <div class="w-full">
                        <select v-model="filters.color_id" @change="applyFilter"
                            class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600">
                    ...
                    <div class="w-full flex col-span-2">
                        <input v-model="filters.priceFrom" type="number" placeholder="Price From"
                            class="block w-full p-2 text-sm text-left rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                            @input="applyFilter" />
                        <input v-model="filters.priceTo" type="number" placeholder="Price To"
                            class="block w-full p-2 text-sm text-left rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                            @input="applyFilter" />
                    </div>
